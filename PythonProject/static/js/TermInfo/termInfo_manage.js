...
function initTermInfoManageTool() {
	termInfo_manage_tool = {
		...
		search: function() {
			$("#termInfo_manage").datagrid("options").queryParams=queryParams; 
			$("#termInfo_manage").datagrid("load");
		},
		exportExcel: function() {
			$("#termInfoQueryForm").form({
			    url:"/TermInfo/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/TermInfo/update/" + rows[0].termId,
					type : "get",
					data : {
