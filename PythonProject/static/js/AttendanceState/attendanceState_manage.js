				...
				if ($("#attendanceStateEditForm").form("validate")) {
					...
					if(!$("#attendanceStateEditForm").form("validate")) {
						...
						$("#attendanceStateEditForm").form({
						    url:"/AttendanceState/update/" + $("#attendanceState_stateId_edit").val(),
						    onSubmit: function(){
								if($("#attendanceStateEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
...
function initAttendanceStateManageTool() {
	attendanceState_manage_tool = {
		...
		search: function() {
			$("#attendanceState_manage").datagrid("options").queryParams=queryParams; 
			$("#attendanceState_manage").datagrid("load");
		},
		exportExcel: function() {
			$("#attendanceStateQueryForm").form({
			    url:"/AttendanceState/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/AttendanceState/update/" + rows[0].stateId,
					type : "get",
					data : {
